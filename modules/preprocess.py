...
def parse_dapa_bidders(df: pd.DataFrame) -> dict:
    """
    방위사업청 국내조달 입찰참여업체정보 파싱.
    반환: {총입찰업체수, 고유업체수, 허핀달지수, 공급업체다양성지수}
    - 허핀달지수(HHI): 낮을수록 독점 없음(경쟁적), 높을수록 집중
    - 공급업체다양성지수: 0~100, 높을수록 다양 (위험 낮음)
    """
    df = df.copy()
    total = len(df)
    if total == 0:
        return {"총입찰업체수": 0, "고유업체수": 0, "허핀달지수": 1.0, "공급업체다양성지수": 0.0}

    # 업체명 컬럼 찾기
    name_col = None
    for c in df.columns:
        if "업체" in c or "업체명" in c:
            name_col = c
            break
    if name_col is None:
    ...
    # 허핀달-허쉬만 지수 (HHI): 각 업체 점유율 제곱합
    counts = df[name_col].value_counts()
    shares = counts / total
    hhi = float((shares ** 2).sum())  # 0~1, 낮을수록 분산

    # 공급업체다양성지수: HHI 역수 기반 0~100 스케일
    # HHI=1(독점)→0점, HHI=1/N(완전경쟁)→100점
    min_hhi = 1 / max(unique_count, 1)
    diversity = float(((1 - hhi) / (1 - min_hhi + 1e-9)) * 100)
    diversity = min(max(diversity, 0), 100)

    return {
        "총입찰업체수": total,
        "고유업체수": unique_count,
        "허핀달지수": round(hhi, 6),
        "공급업체다양성지수": round(diversity, 2),
    }
