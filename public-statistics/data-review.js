...
function buildDataReview(data, agencies, report, year, province, offices) {
  ...
  const normalizeProvince = value => value.replace(/\s*\(.*\)/,'').trim().replace('강원특별자치도','강원도').replace('전북특별자치도','전라북도');
  const extra = agencies.filter(d=>d.extra_kind).map(d=>({...d, selectedRows:d.extra_kind==='catalog' ? [] : d.rows.filter(r=>d.extra_kind==='population' ? related.includes(normalizeProvince(r[0])) : selectedOffices.includes(r[0].replace(/[\s·]/g,'')))}));
  ...
  const verified = !!(data && report?.status === 'passed' && report.source_sha256 === data.source_sha256 && report.row_count === data.rows.length && Number.isFinite(Date.parse(report.checked_at)) && agencies.length === 11 && agencies.every(d => report.agency_sources?.some(s => s.source === d.source && s.sha256 === d.sha256 && s.source_rows === d.source_rows)));
  const checks = [{label:'병무청 선택 연도·관할 값',ok:complete},{label:'질병청 지역행·방위사업청 자료 연결',ok:linked},{label:'배포 보고서와 12종 원본 식별정보 일치',ok:verified},{label:'기관 간 공통 기준일·단위 확인',ok:false}];
  return {extra, related, rows, health, regional, procurement, supplier, supplierRows, present, expected:selectedOffices.length*fields.length, checks, passed:checks.filter(c=>c.ok).length};
}
